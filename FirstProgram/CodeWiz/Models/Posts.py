...
class Posts:
    ...
    def inser_post(self,data):
        insertpost = self.posts.insert({'username': data.username, 'content': data.content, 'date_added': datetime.datetime.now()})
        return True
    ...
    def get_all_posts(self):
        ...
        for post in all_posts:
            ...
            post['timestamp'] = humanize.naturalday(datetime.datetime.now() - post['date_added']) #if it is key error, mean some table doens't have that data
    ...
    def add_comment(self,comment):
        insert_comment = self.comments.insert({'post_id': comment['post_id'],'username': comment['username'], 'content': comment['content'], 'date_added': datetime.datetime.now()})
